...
public class FindStringInArray {
	...
	/*
	 * Use ordinary binary search, but when you hit an empty string, advance to
	 * the next nonempty string; if there is no next non-empty string, search
	 * the left half.
	 */
	...
	public int search(String[] strings, String str, int first, int last) {
		...
		int mid = (last + first) >> 1;// (last+first)/2
		while (strings[mid] == "") {

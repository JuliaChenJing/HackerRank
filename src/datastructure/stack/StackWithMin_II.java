...
public class StackWithMin_II extends Stack<Integer> {
	/*
	 * There’s just one issue with solution I:if we have a large stack, we
	 * waste a lot of space by keeping track of the min for every single
	 * element. Can we do better?
	 * 
	 ...
	 * local min, then we’re keeping a lot of duplicate data. By having the
	 * mins kept in a separate stack, we don’t have this duplicate data
	 * (although we do use up a lot of extra space because we have a stack node

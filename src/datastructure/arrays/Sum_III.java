 ...
 * 设计一个类，包含如下两个成员函数：
   Save（int input）：插入一个整数到一个整数集合里
    Test(int target)：检验是否存在两个数和为输出值。如果存在这两个数，则返回true；否则返回false
    
      提问 :在这个整数集合里是否允许有相同值的元素？
      允许。
    
     思考
      这道面试题是“两个数之和”的变体，但必须注意的是，整数集合里的数是可能有重复的。
      如果Test函数的输入值target为集合里某个数的两倍，我们该如何判断呢？在“两数之和II”的哈希表里存放<值，下标>，
      而在这里就需要改变为记录每个数出现的次数，在哈希表里存放 <值，个数>。如果某数两倍为target值，
      那么只有该数出现两次或两次以上，才能返回true。
 */
